...
export class CraftedItemRenderer {
    ...
    public renderItemToTexture(
        ...
        const { offsetX, offsetY } =
            this.calculateCenteringOffset(detailsToDraw);
    ...
    public getImageDataUrl(
        sourceRT: GameObjects.RenderTexture
    ): Promise<string | null> {
        return new Promise((resolve) => {
            ...
            this.scene.game.events.once(Phaser.Core.Events.POST_STEP, () => {
                if (!sourceRT || !sourceRT.texture || !sourceRT.active) {
                    ...
                    console.warn(
                        "RenderTexture not available or inactive for getImageDataUrl."
                    );
                    resolve(null);
                    return;
                }
                try {
                    ...
                    console.error(
                        "Error getting base64 data from RenderTexture:",
                        error
                    );
                    resolve(null);
                }
    ...
    private estimateAllPartDimensions(recipeItem: RecipeItem): void {
        ...
        uniquePartTypes.forEach((partType) => {
            ...
            if (hasDetails && isFinite(minX)) {
                ...
                console.warn(
                    `Could not estimate dimensions for part ${partType}, using default 1x1.`
                );
            }
        });
    }
    ...
    private prepareDetailsForDrawing(
        item: LootItem,
            ...
            sortedDetailSockets.forEach((detailSocket) => {
                ...
                if (foundDetailId) {
                    ...
                    detailsToDraw.push({
                        ...
                        zIndex:
                            partSocket.pinpoint.zIndex +
                            detailSocket.pinpoint.zIndex / 100, // Combine zIndices for sorting later
                    });
                }
            });
        });
