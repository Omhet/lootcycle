 ...
 * @param sellPriceCoefficient The sell price coefficient of the junk detail
 ...
 */
function getDistributionFromChanceTable(
    totalCount: number,
    ...
    const totalChance = Object.values(chanceTable).reduce(
...
export function getJunkPortion(
        ...
        return detail.suitableForRecipeDetails.some((recipeDetailType) => {
            ...
            return favoriteRecipeObjects.some((recipe) => {
                ...
                return recipe.sockets.some((socket) => {
                    const recipeParts =
                        lootConfig.recipeParts[socket.acceptType] || [];
                    return recipeParts.some((recipePart) => {
                        // Check if any part socket accepts this detail type
                        return recipePart.sockets.some(
                            (s) => s.acceptType === recipeDetailType
                        );
                    });
                });
            });
    ...
    const qualityChanceTable =
        chanceTables.qualityChanceTables[qualityChanceLevel] ||
        chanceTables.qualityChanceTables[1];

    const rarityChanceTable =
        chanceTables.rarityChanceTables[rarityChanceLevel] ||
    ...
    const qualityDistribution = getDistributionFromChanceTable(
        portionSize,
        qualityChanceTable
    );
    const rarityDistribution = getDistributionFromChanceTable(
        portionSize,
        rarityChanceTable
    );
    ...
    for (const quality of Object.values(Quality)) {
        ...
        for (const rarity of Object.values(Rarity)) {
            ...
            const groupCount = Math.min(
                Math.ceil(remainingQualityCount * proportion),
                remainingQualityCount
            );
