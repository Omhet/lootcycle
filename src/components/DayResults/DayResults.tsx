import s from "./DayResults.module.css";

type DayResultsProps = {
    dayNumber: number;
    balance: number;
    junkRecycled: number;
    junkBurnt: number;
    lootCrafted: number;
    lootScrewedUp: number;
    profit: number;
    clawWearout: number;
    cauldronWearout: number;
    stoveWearout: number;
    onClose: () => void;
};

export const DayResults = ({
    dayNumber,
    balance,
    junkRecycled,
    junkBurnt,
    lootCrafted,
    lootScrewedUp,
    profit,
    clawWearout,
    cauldronWearout,
    stoveWearout,
    onClose,
}: DayResultsProps) => {
    return (
        <div className={s.dayResultsContainer}>
            <div className={s.header}>
                <div className={s.headerContent}>
                    {/* <span className={s.headerTitle}>
                        Another excellent day!
                    </span>
                    <span className={s.headerDescription}>
                        Check out your stats for day {dayNumber}
                    </span> */}
                    <span className={s.headerTitle}>day {dayNumber}</span>
                </div>
                <div>
                    <span>Balance: {balance}</span>
                </div>
            </div>
            <div className={s.stats}>
                <span>Junk recycled: {junkRecycled} items</span>
                <span>Profit: {profit} gold</span>
                <span>Junk burnt: {junkBurnt} items</span>
                <span>Claw wearout: {clawWearout}%</span>
                <span>Loot crafted: {lootCrafted} items</span>
                <span>Cauldron wearout: {cauldronWearout}%</span>
                <span>Loot screwed up: {lootScrewedUp} items</span>
                <span>Stove wearout: {stoveWearout}%</span>
            </div>
            <button className={s.button} onClick={onClose}>
                Great!
            </button>
        </div>
    );
};
